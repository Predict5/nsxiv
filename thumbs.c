...
static bool tns_cache_whitelisted(tns_t *tns, const char *filepath) {
  ...
  for (i = 0; i < tns->filters_cnt; ++i) {
    ...
    if ((f->recursive ? (dir_len >= f->len) : (dir_len == f->len)) &&
        memcmp(filepath, f->path, f->len) ==
            0) { /* in blacklist mode, finding a match means we shouldn't cache
                  */
...
static void tns_cache_write(tns_t *tns, Imlib_Image im, const char *filepath,
                            bool force) {
  char *cfile, *dirend;
  int tmpfd;
  struct stat cstats, fstats;
  ...
  if (options->tns_filters != NULL && options->tns_filters[0] != '\0') {
    ...
    for (tok = strtok_r(s, ":", &save); tok != NULL;
      ...
      if (tns->filters_cnt == allocated) {
        ...
        tns->filters =
            erealloc(tns->filters, allocated * sizeof(*tns->filters));
      }
      ...
      if (f->path == NULL) {
        error(EXIT_FAILURE, errno, "--cache-%s: `%s`",
              tns->filters_is_blacklist ? "deny" : "allow", tok + f->recursive);
      }
      f->len = strlen(f->path);
    }
    free(s);
...
bool tns_zoom(tns_t *tns, int d) {
  ...
  tns->bw = MIN(tns->bw, 4);
  tns->dim =
      thumb_sizes[tns->zl] + THUMB_BORDER_MULTIPLIER * tns->bw + THUMB_PADDING;
